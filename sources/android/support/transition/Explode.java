...
public class Explode extends Visibility {
    ...
    public Animator onAppear(ViewGroup sceneRoot, View view, TransitionValues startValues, TransitionValues endValues) {
        ...
        calculateOut(sceneRoot, bounds, explode.mTempLoc);
        return TranslationAnimationCreator.createAnimation(view, transitionValues, bounds.left, bounds.top, endX + ((float) explode.mTempLoc[0]), endY + ((float) explode.mTempLoc[1]), endX, endY, sDecelerate);
    }
    ...
    public Animator onDisappear(ViewGroup sceneRoot, View view, TransitionValues startValues, TransitionValues endValues) {
        ...
        float endX = startX;
        float endY = startY;
        int[] interruptedPosition = (int[]) transitionValues.view.getTag(C0082R.id.transition_position);
        ...
        calculateOut(sceneRoot, bounds, explode.mTempLoc);
        return TranslationAnimationCreator.createAnimation(view, transitionValues, viewPosX, viewPosY, startX, startY, endX + ((float) explode.mTempLoc[0]), endY + ((float) explode.mTempLoc[1]), sAccelerate);
    }
