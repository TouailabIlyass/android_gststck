...
public class ChangeImageTransform extends Transition {
    ...
    private static final TypeEvaluator<Matrix> NULL_MATRIX_EVALUATOR = new C00721();
    ...
    static class C00721 implements TypeEvaluator<Matrix> {
        C00721() {
        }
    ...
    private static Matrix copyImageMatrix(ImageView view) {
        switch (C00743.$SwitchMap$android$widget$ImageView$ScaleType[view.getScaleType().ordinal()]) {
            case 1:
    ...
    private static Matrix centerCropMatrix(ImageView view) {
        ...
        int tx = Math.round((((float) imageViewWidth) - (((float) imageWidth) * maxScale)) / 1073741824);
