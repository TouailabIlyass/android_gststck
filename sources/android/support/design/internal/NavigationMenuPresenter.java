...
public class NavigationMenuPresenter implements MenuPresenter {
    ...
    final OnClickListener mOnClickListener = new C00221();
    ...
    private class NavigationMenuAdapter extends Adapter<ViewHolder> {
                    ...
                    if (i < totalSize) {
                        ...
                        if (item.hasSubMenu()) {
                            ...
                            if (subMenu.hasVisibleItems()) {
                                ...
                                r0.mItems.add(new NavigationMenuTextItem(item));
                                boolean subMenuHasIcon = false;
                                int subMenuStart = r0.mItems.size();
                                ...
                                for (int j = 0; j < size; j++) {
                                    ...
                                    if (subMenuItem.isVisible()) {
                                        ...
                                        r0.mItems.add(new NavigationMenuTextItem(subMenuItem));
                                    }
                                }
                                if (subMenuHasIcon) {
                                    appendTransparentIconIfMissing(subMenuStart, r0.mItems.size());
                                }
                            }
                        } else {
                            int currentGroupId2 = item.getGroupId();
                            if (currentGroupId2 != currentGroupId) {
                                currentGroupStart = r0.mItems.size();
                                ...
                                appendTransparentIconIfMissing(currentGroupStart, r0.mItems.size());
                            }
                            NavigationMenuTextItem textItem = new NavigationMenuTextItem(item);
                            textItem.needsEmptyIcon = currentGroupHasIcon;
                            r0.mItems.add(textItem);
                            currentGroupId = currentGroupId2;
                        }
                        i++;
                    } else {
        ...
        public void setCheckedItem(MenuItemImpl checkedItem) {
            if (this.mCheckedItem != checkedItem) {
                if (checkedItem.isCheckable()) {
        ...
        public void restoreInstanceState(Bundle state) {
            ...
            if (checkedItem != 0) {
                ...
                for (i = 0; i < size; i++) {
                    item = (NavigationMenuItem) this.mItems.get(i);
                    if (item instanceof NavigationMenuTextItem) {
                        menuItem = ((NavigationMenuTextItem) item).getMenuItem();
                ...
                for (i = 0; i < size; i++) {
                    ...
                    if (item instanceof NavigationMenuTextItem) {
                        ...
                        if (menuItem != null) {
                            ...
                            if (actionView != null) {
                                ParcelableSparseArray container = (ParcelableSparseArray) actionViewStates.get(menuItem.getItemId());
                                if (container != null) {
    ...
    private static class NormalViewHolder extends ViewHolder {
        public NormalViewHolder(LayoutInflater inflater, ViewGroup parent, OnClickListener listener) {
            super(inflater.inflate(C0019R.layout.design_navigation_item, parent, false));
    ...
    private static class SeparatorViewHolder extends ViewHolder {
        public SeparatorViewHolder(LayoutInflater inflater, ViewGroup parent) {
            super(inflater.inflate(C0019R.layout.design_navigation_item_separator, parent, false));
        }
    }

    private static class SubheaderViewHolder extends ViewHolder {
        public SubheaderViewHolder(LayoutInflater inflater, ViewGroup parent) {
            super(inflater.inflate(C0019R.layout.design_navigation_item_subheader, parent, false));
        }
    }
    ...
    public void initForMenu(Context context, MenuBuilder menu) {
        ...
        this.mMenu = menu;
        this.mPaddingSeparator = context.getResources().getDimensionPixelOffset(C0019R.dimen.design_navigation_separator_vertical_padding);
    }

    public MenuView getMenuView(ViewGroup root) {
        if (this.mMenuView == null) {
            this.mMenuView = (NavigationMenuView) this.mLayoutInflater.inflate(C0019R.layout.design_navigation_menu, root, false);
            if (this.mAdapter == null) {
                this.mAdapter = new NavigationMenuAdapter();
            }
            this.mHeaderLayout = (LinearLayout) this.mLayoutInflater.inflate(C0019R.layout.design_navigation_item_header, this.mMenuView, false);
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (this.mHeaderLayout != null) {
            hierarchy = new SparseArray();
            this.mHeaderLayout.saveHierarchyState(hierarchy);
            state.putSparseParcelableArray(STATE_HEADER, hierarchy);
        }
        return state;
    }
