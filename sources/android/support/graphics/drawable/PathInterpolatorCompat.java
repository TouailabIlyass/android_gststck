...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (((double) Math.abs(this.mX[0])) <= 1.0E-5d && ((double) Math.abs(this.mY[0])) <= 1.0E-5d && ((double) Math.abs(this.mX[numPoints - 1] - 1.0f)) <= 1.0E-5d) {
            if (((double) Math.abs(this.mY[numPoints - 1] - 1.0f)) <= 1.0E-5d) {
                float prevX = 0.0f;
                float x = 0.0f;
                while (i < numPoints) {
                    int componentIndex = x + 1;
                    x = this.mX[x];
                    if (x < prevX) {
        ...
        stringBuilder3.append("The Path must start at (0,0) and end at (1,1) start: ");
        stringBuilder3.append(this.mX[0]);
        stringBuilder3.append(",");
        stringBuilder3.append(this.mY[0]);
        stringBuilder3.append(" end:");
        stringBuilder3.append(this.mX[numPoints - 1]);
        stringBuilder3.append(",");
        stringBuilder3.append(this.mY[numPoints - 1]);
