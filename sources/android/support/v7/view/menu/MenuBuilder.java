...
public class MenuBuilder implements SupportMenu {
    ...
    public int addIntentOptions(int group, int id, int categoryOrder, ComponentName caller, Intent[] specifics, Intent intent, int flags, MenuItem[] outSpecificItems) {
        ...
        while (i < N) {
            ...
            MenuItem item = r0.add(group, id, categoryOrder, ri.loadLabel(pm)).setIcon(ri.loadIcon(pm)).setIntent(rintent);
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        boolean z = true;
        if (!shortcutsVisible || this.mResources.getConfiguration().keyboard == 1 || !this.mResources.getBoolean(C0249R.bool.abc_config_showMenuShortcutsWhenKeyboardPresent)) {
            z = false;
        }
    ...
    void findItemsWithShortcutForKey(List<MenuItemImpl> items, int keyCode, KeyEvent event) {
        ...
        if (keyEvent.getKeyData(possibleChars) || i == 67) {
            int N = menuBuilder.mItems.size();
            int i2 = 0;
            while (i2 < N) {
                Object obj;
                Object obj2;
                MenuItemImpl item = (MenuItemImpl) menuBuilder.mItems.get(i2);
                if (item.hasSubMenu()) {
                ...
                if (((modifierState & SupportMenu.SUPPORTED_MODIFIERS_MASK) == (SupportMenu.SUPPORTED_MODIFIERS_MASK & (qwerty ? item.getAlphabeticModifiers() : item.getNumericModifiers()))) && shortcutChar != '\u0000') {
