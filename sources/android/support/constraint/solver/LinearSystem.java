...
public class LinearSystem {
    ...
    private final int optimize(Row goal, boolean b) {
        ...
        while (!done) {
            ...
            tries++;
            if (tries >= 2 * linearSystem.mNumColumns) {
                return tries;
            }
            if (goal.getKey() != null) {
                linearSystem.mAlreadyTestedCandidates[goal.getKey().id] = true;
            }
            SolverVariable pivotCandidate = goal.getPivotCandidate(linearSystem, linearSystem.mAlreadyTestedCandidates);
            if (pivotCandidate != null) {
                if (linearSystem.mAlreadyTestedCandidates[pivotCandidate.id]) {
                    return tries;
                }
                linearSystem.mAlreadyTestedCandidates[pivotCandidate.id] = true;
            }
            if (pivotCandidate != null) {
                ArrayRow current;
                int pivotRowIndex = -1;
                float min = Float.MAX_VALUE;
                for (int i2 = 0; i2 < linearSystem.mNumRows; i2++) {
                    current = linearSystem.mRows[i2];
                ...
                if (pivotRowIndex > -1) {
                    current = linearSystem.mRows[pivotRowIndex];
        ...
        if (infeasibleSystem) {
            ...
            while (!done) {
                ...
                while (i3 < linearSystem.mNumRows) {
                    ArrayRow current = linearSystem.mRows[i3];
                    if (current.variable.mType != Type.UNRESTRICTED) {
                        if (!current.isSimpleDefinition) {
                            if (current.constantValue < f) {
                                int j = 1;
                                while (j < linearSystem.mNumColumns) {
                                    SolverVariable candidate = linearSystem.mCache.mIndexedVariables[j];
                                    float a_j = current.variables.get(candidate);
                                    if (a_j > f) {
                                        int k = 0;
                                        while (k < 7) {
                                            f = candidate.strengthVector[k] / a_j;
                                            if ((f < min && k == r11) || k > r11) {
                                                min = f;
                                                pivotRowIndex = i3;
                                                pivotColumnIndex = j;
                                                strength = k;
                ...
                if (pivotRowIndex != -1) {
                    ArrayRow pivotEquation = linearSystem.mRows[pivotRowIndex];
                    ...
                    pivotEquation.pivot(linearSystem.mCache.mIndexedVariables[pivotColumnIndex]);
