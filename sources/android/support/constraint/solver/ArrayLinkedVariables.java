...
public class ArrayLinkedVariables {
    ...
    SolverVariable chooseSubject(LinearSystem system) {
        ...
        float candidateAmount = 0.0f;
        while (current != -1 && counter < r0.currentSize) {
            float amount = r0.mArrayValues[current];
            SolverVariable variable = r0.mCache.mIndexedVariables[r0.mArrayIndices[current]];
            if (amount < f) {
                if (amount > (-981668463)) {
                    r0.mArrayValues[current] = f;
                    amount = 0.0f;
                    variable.removeFromRow(r0.mRow);
                }
            ...
            current = r0.mArrayNextIndices[current];
            counter++;
        }
        if (unrestrictedCandidate != null) {
    ...
    void updateFromSystem(ArrayRow self, ArrayRow[] rows) {
        ...
        while (current != -1 && counter < r0.currentSize) {
            ...
            if (variable.definitionId != -1) {
                float value = r0.mArrayValues[current];
                remove(variable, true);
                ...
                if (!definition.isSimpleDefinition) {
                    ...
                    int definitionCounter = 0;
                    while (definitionCurrent != -1 && definitionCounter < definitionVariables.currentSize) {
                        add(r0.mCache.mIndexedVariables[definitionVariables.mArrayIndices[definitionCurrent]], definitionVariables.mArrayValues[definitionCurrent] * value, true);
                        definitionCurrent = definitionVariables.mArrayNextIndices[definitionCurrent];
                        definitionCounter++;
                    }
                }
